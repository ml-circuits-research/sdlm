...
When an answer-only request contains a successful current-quantity query, SOP presentation must suppress successful numerical adjustment confirmations in that request. The final question answers retain their order; standalone adjustments and failed updates remain visible. Every suppressed update must retain its full structured result, explanation and [assumptions](../wiki.html#definition-assumption). Explained mode keeps the intermediate confirmations.
