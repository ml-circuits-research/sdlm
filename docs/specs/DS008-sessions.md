...
[Quantity focus](../wiki.html#definition-quantity-focus) must persist as a bounded identifier list in `SessionSettings.sop`. It records at most sixteen distinct owner/item pairs in recency order; quantities and their supporting [atoms](../wiki.html#definition-atom) must be read from the live knowledge [snapshot](../wiki.html#definition-snapshot). Missing or non-unique values must be excluded from reference resolution. Successful conversation quantity operations may advance focus; failed operations and rolled-back requests must not. A [snapshot](../wiki.html#definition-snapshot) without this setting starts with empty focus. History matching must prevent a repeated gain or loss from being applied again. DS017 owns the selection policy.
